...
function Products() {
  ...
  const fetchProducts = async () => {
    try {
      ...
      const productList = querySnapshot.docs.map((doc) => {
        const data = doc.data();
        // Ensure backward compatibility: if images array doesn't exist, create from single image
  ...
  // FULL PRODUCT DETAIL PAGE - Reviews will show here
  if (selectedProduct) {
    return (
      <div className="py-16 px-6 max-w-7xl mx-auto">
        <button
        ...
        <div className="grid md:grid-cols-2 gap-8">
          <div>
            <img
              src={selectedProduct.images?.[0] || selectedProduct.image || `https://picsum.photos/600/400?random=${selectedProduct.id}`}
        ...
        {/* REVIEWS SECTION - This will now be visible on the full product page */}
        <div className="mt-12">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
        {filteredProducts.length === 0 ? (
          ...
          filteredProducts.map((product) => (
            <div
              key={product.id}
              className="bg-white rounded-xl shadow-lg overflow-hidden hover:shadow-2xl transition relative group"
            >
              {/* Product Image - Click to go to full details */}
              <div 
                onClick={() => setSelectedProduct(product)}
                className="cursor-pointer"
              >
                <img
                  src={product.images?.[0] || product.image || `https://picsum.photos/400/300?random=${product.id}`}
                  alt={product.name}
              ...
              <div className="p-4">
                {/* Product Name - Click to go to full details */}
                <h2 
                  onClick={() => setSelectedProduct(product)}
                  className="text-lg font-semibold line-clamp-1 mb-2 cursor-pointer hover:text-amber-600 transition"
                >
                  {product.name}
                </h2>
                ...
                <p className="text-gray-600 text-sm line-clamp-2 mb-2 mt-2">{product.description}</p>
